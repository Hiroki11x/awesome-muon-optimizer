...
- **[Orth-Dion: Eliminating Geometric Mismatch in Distributed Low-Rank Spectral Optimization](https://arxiv.org/abs/2605.16341)** — Tatsuhiro Nakamori, Laura Gomezjurado Gonzalez, Ganesh Talluri, et al., Preprint, 2026. Identifies Dion's column-normalization step as the source of its gap to full-rank spectral methods — it does not produce the rank-r polar factor the geometry implies, so the update violates the low-rank dual-norm constraint and the rate picks up a factor of sqrt(r) — and substitutes QR orthogonalization of the right factor, with an adaptive-rank variant to offset the added cost; reports validation loss, per-step wall-clock, and direct measurement of the dual-norm factor on Llama 3 pretraining at 320M and 17.1B parameters.
